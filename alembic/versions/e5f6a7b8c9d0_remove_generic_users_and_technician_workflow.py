...
def upgrade() -> None:
    """Cut over to final Resident/BQL schema and remove Technician artifacts."""
    op.execute(
        """
        DO $$
        DECLARE
        ...
        BEGIN
            SELECT count(*) INTO resident_count FROM public.users WHERE role = 'resident';
            SELECT count(*) INTO copied_resident_count FROM public.residents;
            SELECT count(*) INTO coordinator_count FROM public.users WHERE role = 'coordinator';
            SELECT count(*) INTO copied_bql_count FROM public.bql_staff;
            SELECT count(*) INTO membership_count FROM public.user_unit_memberships;
            SELECT count(*) INTO copied_membership_count FROM public.resident_unit_memberships;
            SELECT count(*) INTO invalid_ticket_count
            FROM public.tickets t LEFT JOIN public.residents r ON r.id = t.resident_id
            WHERE r.id IS NULL;
            SELECT count(*) INTO invalid_upload_session_count
            FROM public.ticket_attachment_upload_sessions s LEFT JOIN public.residents r ON r.id = s.resident_id
            WHERE r.id IS NULL;
            SELECT count(*) INTO technician_profile_count FROM public.technician_profiles;
            SELECT count(*) INTO technician_skill_count FROM public.technician_skills;
            SELECT count(*) INTO ticket_assignment_count FROM public.ticket_assignments;
            ...
            IF resident_count <> copied_resident_count
               ...
               OR ticket_assignment_count > 0 THEN
                RAISE EXCEPTION
                    'Cannot cut over actor schema: resident_count=%, copied_resident_count=%, coordinator_count=%, copied_bql_count=%, membership_count=%, copied_membership_count=%, invalid_ticket_count=%, invalid_upload_session_count=%, technician_profile_count=%, technician_skill_count=%, ticket_assignment_count=%',
                    resident_count, copied_resident_count, coordinator_count, copied_bql_count, membership_count,
                    copied_membership_count, invalid_ticket_count, invalid_upload_session_count, technician_profile_count,
                    technician_skill_count, ticket_assignment_count;
            END IF;
        END $$;
        """
    )
    ...
    op.drop_index("ix_ticket_attachment_upload_sessions_owner_status", table_name="ticket_attachment_upload_sessions")
    op.drop_index("ix_notifications_recipient_unread_created_at", table_name="notifications")
    ...
    op.create_index("ix_audit_logs_actor_auth_created_at", "audit_logs", ["actor_auth_user_id", "created_at"])
    op.execute(
        """
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auth' AND table_name = 'users') THEN
                ALTER TABLE public.ticket_status_history
                    ...
                    FOREIGN KEY (changed_by_auth_user_id) REFERENCES auth.users(id) ON DELETE SET NULL;
                ALTER TABLE public.notifications
                    ADD CONSTRAINT fk_notifications_recipient_auth_users
                    FOREIGN KEY (recipient_auth_user_id) REFERENCES auth.users(id) ON DELETE RESTRICT;
    ...
    op.execute(
        ...
        ON bql_staff FOR SELECT TO authenticated
        USING (id = (select auth.uid()));

        CREATE POLICY rls_resident_unit_memberships_select_own_active
        ON resident_unit_memberships FOR SELECT TO authenticated
        USING (resident_id = (select auth.uid()) AND is_active = true);
        ...
        USING (
            EXISTS (SELECT 1 FROM tickets WHERE tickets.id = ticket_status_history.ticket_id)
        );
        ...
        CREATE POLICY rls_business_tables_deny_client_mutation
        ON residents FOR ALL TO anon, authenticated USING (false) WITH CHECK (false);
        """
    )
    for table_name in (
        "residents",
...
def downgrade() -> None:
    """This breaking actor cutover is intentionally not reversible."""
    raise NotImplementedError("Downgrade is not supported for the Resident/BQL actor cutover.")
