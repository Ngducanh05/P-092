...
def upgrade() -> None:
    """Add Resident/BQL profile tables and copied actor columns."""
    op.create_table(
        "residents",
    ...
    op.execute(
        ...
        FOR EACH ROW EXECUTE FUNCTION public.prevent_actor_profile_conflict();
    ...
    op.execute(
        ...
        BEGIN
            SELECT count(*) INTO technician_count FROM public.users WHERE role = 'technician';
            SELECT count(*) INTO admin_count FROM public.users WHERE role = 'admin';
            SELECT count(*) INTO resident_without_phone_count
            FROM public.users
            WHERE role = 'resident'
              AND (phone_number IS NULL OR phone_number !~ '^\\+[1-9][0-9]{6,14}$');
            SELECT count(*) INTO coordinator_without_email_count
            FROM public.users
            WHERE role = 'coordinator' AND email IS NULL;

            IF technician_count > 0 OR admin_count > 0 OR resident_without_phone_count > 0 OR coordinator_without_email_count > 0 THEN
                RAISE EXCEPTION
                    'Cannot migrate actor profiles: technician_count=%, admin_count=%, resident_without_phone_count=%, coordinator_without_email_count=%',
                    technician_count, admin_count, resident_without_phone_count, coordinator_without_email_count;
            END IF;
        END $$;
        """
    )
    ...
    op.execute(
        """
        INSERT INTO public.residents (id, phone_number, full_name, is_active, created_at, updated_at)
        SELECT id, phone_number, full_name, is_active, created_at, updated_at
        FROM public.users
        WHERE role = 'resident'
        ON CONFLICT (id) DO NOTHING;

        INSERT INTO public.bql_staff (id, email, full_name, is_active, created_at, updated_at)
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("resident_id", "unit_id", name="uq_resident_unit_memberships_resident_unit"),
    )
    op.create_index(
    ...
    op.execute(
        ...
        INSERT INTO public.resident_unit_memberships
            (id, resident_id, unit_id, is_active, linked_at, unlinked_at, created_at, updated_at)
        SELECT id, user_id, unit_id, is_active, linked_at, unlinked_at, created_at, updated_at
    ...
    op.add_column("ticket_attachment_upload_sessions", sa.Column("resident_id", sa.Uuid(), nullable=True))
    op.execute("UPDATE ticket_attachment_upload_sessions SET resident_id = owner_user_id")
    op.alter_column("ticket_attachment_upload_sessions", "resident_id", nullable=False)
    op.create_foreign_key(
    ...
    op.add_column("ticket_status_history", sa.Column("changed_by_auth_user_id", sa.Uuid(), nullable=True))
    op.execute("UPDATE ticket_status_history SET changed_by_auth_user_id = changed_by_user_id")
    op.add_column("notifications", sa.Column("recipient_auth_user_id", sa.Uuid(), nullable=True))
    op.execute("UPDATE notifications SET recipient_auth_user_id = recipient_user_id")
    op.alter_column("notifications", "recipient_auth_user_id", nullable=False)
    op.add_column("audit_logs", sa.Column("actor_auth_user_id", sa.Uuid(), nullable=True))
    op.execute("UPDATE audit_logs SET actor_auth_user_id = actor_user_id")
...
def downgrade() -> None:
    """Remove additive Resident/BQL structures."""
    ...
    op.drop_index("ix_ticket_attachment_upload_sessions_resident_status", table_name="ticket_attachment_upload_sessions")
    op.drop_constraint(
        "fk_ticket_attachment_upload_sessions_resident_id_residents",
        "ticket_attachment_upload_sessions",
        type_="foreignkey",
    ...
    op.drop_constraint("fk_tickets_resident_id_residents", "tickets", type_="foreignkey")
