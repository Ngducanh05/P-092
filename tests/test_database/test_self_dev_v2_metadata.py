...
def test_canonical_tables_present_and_technician_tables_absent():
    tables = set(Base.metadata.tables)
    assert {
        "user_profiles", "resident_profiles", "buildings", "floors", "units",
        "location_types", "locations", "categories", "scoring_rule_versions",
        "tickets", "ticket_attachments", "ai_analysis_runs", "ticket_status_history",
        "information_requests", "incident_cases", "incident_case_members",
        "notifications", "audit_logs",
    } <= tables
    assert not {"technician_profiles", "technician_skills", "ticket_assignments", "bql_staff", "residents"} & tables
