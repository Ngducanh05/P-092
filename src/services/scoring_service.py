"""Transparent priority scoring defined in Self_Dev_Docs/Logic_xử_lý_chính_v2.md.
...
CANONICAL_LOCATION_BONUS = {
    Category.LOCK_DOOR: {"MAIN_DOOR": 30, "SECURITY_DOOR": 30},
    Category.COMMON_LIGHT: {"FIRE_EXIT": 25},
}
...
class ScoringService:
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        self.location_matrix = {category: dict(values) for category, values in CANONICAL_LOCATION_BONUS.items()}
    ...
    def _load_config(self, config: dict[str, Any]) -> None:
        ...
        for category_key, location_values in (config.get("location_bonus") or {}).items():
            self.location_matrix[Category(category_key)] = {
    ...
    def location_bonus(self, category: Category, location_type_code: str | None) -> int:
        code = (location_type_code or "").upper()
        return int(self.location_matrix.get(category, {}).get(code, 0))
